...
+(DBManager*)sharedInstance{
    ...
    dispatch_once(&oncePredicate, ^{
        _sharedInstance = [[DBManager alloc] initDatabase];
    });
...
-(instancetype)initDatabase{
    ...
    if (self) {
        ...
        self.m_documentsDirectory = [paths objectAtIndex:0];
...
-(void)copyDatabaseIntoDocumentsDirectory{
    ...
    NSString *destinationPath = [self.m_documentsDirectory stringByAppendingPathComponent:m_databaseFileName];
    if (![[NSFileManager defaultManager] fileExistsAtPath:destinationPath]) {
        // The database file does not exist in the documents directory, so copy it from the main bundle now.
        NSString *sourcePath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:m_databaseFileName];
        NSError *error;
        [[NSFileManager defaultManager] copyItemAtPath:sourcePath toPath:destinationPath error:&error];
...
-(void)runQuery:(const char *)query isQueryExecutable:(BOOL)queryExecutable{
    ...
    NSString *databasePath = [self.m_documentsDirectory stringByAppendingPathComponent:m_databaseFileName];
